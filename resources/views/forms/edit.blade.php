...
<div class="row">
    <div class="card">
                    ...
                    <div class="row bg-light p-2">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                              ...
                              <li class="breadcrumb-item active" aria-current="page">{{__('Edit a Form')}}</li>
                            </ol>
                        </nav>
                    </div>
        ...
        <form action="{{ route('forms.update',$formDetails->id) }}" method="POST" class="needs-validation" novalidate id="formubmit_{{$formDetails->id}}">
        @csrf
        @method('PUT')
            ...
            <div class="card-body">
               ...
               <div class="form-group fv-row mb-10 fv-plugins-icon-container">
                    <label  class="d-flex required align-items-center fs-6 fw-bold mb-2">{{__(' Name:') }} </label>
                    <input type="text" class="form-control" name="formName" id="formName" value ="{{$formDetails->name}}">                    
                    <label  class="d-flex required align-items-center fs-6 fw-bold mb-2">Description</label>
                    <textarea name='formDescription'   class='form-control '  id=""  rows='3' > {{$formDetails->description}}</textarea>                   
               </div>
                ...
                <?php $addClass = ""; 
                    ...
                    @switch($val)
                      ...
                      @foreach($getAdiitionalLogic as $key1 => $addlogic)                           
                            ...
                            @endif                                                   
                        @endforeach  
                      <div class='form-group fv-row mb-10  fv-plugins-icon-container {{$val->label_name}}'  data-id="{{$val->id}}" data-attr="{{$val->label_name}}">
                      ...
                      <div class='form-group fv-row mb-10 fv-plugins-icon-container {{$val->label_name}}'  data-id="{{$val->id}}" data-attr="{{$val->label_name}}">
                       ...
                       <label class='d-flex align-items-center fs-6 fw-bold mb-2'>{{$val->label_name}}</label>  
                        ...
                        @endforeach     
                            ...
                            <input type='checkbox'  {{$checkedVal}}  class='{{$elementsOptions->field_classname}} custom_fieldsCheck {{$addClass}}' data-id="{{$val->id}}"  data-addition-logic="{{$elementsOptions->id}}"  data-visibility="{{$addClass}}"  name='{{$elementsOptions->field_id}}'   id="{{$elementsOptions->field_id}}" required="{{$elementsOptions->required_field}}">
                    ...
                    @case($val->fieldType == "radio")
                      ...
                      <div class='form-group fv-row mb-10 fv-plugins-icon-container {{$val->label_name}} ' data-id="{{$val->id}}"  data-attr="{{$val->label_name}}">
                       ...
                       @if($elementsOptions->custom_field_id == $val->id)
                        ...
                        <input type='radio' {{$checkedVal}}  class='{{$elementsOptions->field_classname}} {{$addClass}}' data-id="{{$val->id}}"  data-visibility="{{$addClass}}"  data-addition-logic="{{$elementsOptions->id}}" name='{{$elementsOptions->field_id}}'   id="{{$elementsOptions->field_id}}" required="{{$elementsOptions->required_field}}">
                        <label>{{$elementsOptions->field_option}}</label>
                        @endif
                        @endif
                        @endforeach
            ...
            <button type="reset" class="btn btn-danger">{{__('Reset')}}</button>         
                <button type="submit" id="updateEditBtn" class="btn btn-primary"> {{__('Update')}}</button>
            </div>
        </form>
    </div>
</div>
...
@section('custom-script')
    ...
    <script src="{{ url('js/custom/edit_custom_form.js') }}"></script>
